...
export async function hydrateProjects(projectRows) {
  ...
  const [comments, milestones, tasks, monthlyTasks, teamMembers] = await Promise.all([
    ...
    query("SELECT * FROM project_team_members WHERE project_id = ANY($1::uuid[]) ORDER BY position ASC, id ASC", [ids]),
  ]);
  ...
  return projectRows.map((project) => ({
    ...
    teamMembers: teamMembers.rows.filter((item) => item.project_id === project.id).map((item) => ({
      id: item.id,
