    ...
    <main className="min-h-screen bg-slate-50">
      <header className="sticky top-0 z-20 border-b border-slate-200 bg-white/90 backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
          <div><p className="text-xs font-bold uppercase tracking-[0.2em] text-[#F34F1F]">Teacher dashboard</p><h1 className="text-xl font-bold text-slate-900">{user.name}</h1></div>
      ...
      <section className="mx-auto max-w-7xl space-y-6 px-6 py-8">
        ...
        <div className="grid gap-6 xl:grid-cols-[0.95fr_1.05fr]">
          <div className="space-y-6">
            ...
            <Card title="Project review queue" icon={Search}><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search projects or students" className="mb-4 w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm outline-none" /><div className="space-y-3">{filtered.map((project) => <button key={project.id} onClick={() => setActive(project)} className={(active?.id === project.id ? "border-[#F34F1F] bg-orange-50" : "border-slate-100 bg-slate-50 hover:bg-slate-100") + " w-full rounded-2xl border p-4 text-left transition"}><div className="flex items-start justify-between gap-3"><div><p className="font-bold text-slate-900">{project.title}</p><p className="mt-1 text-sm text-slate-500">{project.studentName} • {project.group}</p></div><span className={(statusClass[project.status] || "bg-slate-100 text-slate-600") + " rounded-full px-2.5 py-1 text-xs font-bold uppercase"}>{project.status}</span></div><div className="mt-4 h-2 rounded-full bg-white"><div className="h-2 rounded-full bg-[#F34F1F]" style={{ width: project.progress + "%" }} /></div></button>)}</div></Card>
            <Card title="Pending proof submissions" icon={BellRing}>{pendingTasks.length ? pendingTasks.map((task) => <div key={task.id} className="mb-4 rounded-2xl bg-slate-50 p-4"><p className="font-bold text-slate-900">{task.projectTitle}</p><p className="text-sm text-slate-500">{task.studentName} • {task.group} • Month {task.monthNumber}</p><p className="mt-2 text-sm text-slate-600">Screenshot: {task.screenshotName || "Not attached"}</p><p className="text-sm text-slate-600">Video: {task.videoName || "Not attached"}</p><textarea value={taskFeedback[task.id] || ""} onChange={(event) => setTaskFeedback((prev) => ({ ...prev, [task.id]: event.target.value }))} rows={3} placeholder="Add verification feedback" className="mt-3 w-full rounded-2xl border border-slate-200 bg-white p-3 text-sm outline-none focus:border-[#F34F1F]" /><div className="mt-3 flex gap-3"><button onClick={() => verifyTask(task.id, "verified")} className="rounded-xl bg-emerald-600 px-4 py-2 text-sm font-bold text-white">Green Tick</button><button onClick={() => verifyTask(task.id, "rejected")} className="rounded-xl bg-rose-600 px-4 py-2 text-sm font-bold text-white">Cross Mark</button></div></div>) : <p className="text-sm text-slate-500">No pending proof submissions.</p>}</Card>
          </div>

          <div className="space-y-6">
            <div className="rounded-[2rem] bg-white p-8 shadow-sm">{active ? <><span className={(statusClass[active.status] || "bg-slate-100 text-slate-600") + " rounded-full px-3 py-1 text-xs font-bold uppercase"}>{active.status}</span><h2 className="mt-4 text-3xl font-bold text-slate-900">{active.title}</h2><p className="mt-2 text-slate-500">{active.studentName} • {active.domain} • Submitted {active.submittedAt}</p><div className="mt-8 grid gap-5 md:grid-cols-2"><Block title="Abstract" text={active.abstract} /><Block title="Problem statement" text={active.problemStatement} /><Block title="Proposed solution" text={active.proposedSolution} /><Block title="Tech stack" text={active.techStack.join(", ")} /></div><div className="mt-8 rounded-2xl bg-slate-50 p-5"><label className="mb-2 block text-sm font-bold text-slate-700">Proposal feedback</label><textarea value={comment} onChange={(event) => setComment(event.target.value)} rows={4} placeholder="Write feedback for the student..." className="w-full resize-none rounded-2xl border border-slate-200 bg-white p-4 text-sm outline-none focus:border-[#F34F1F]" /><div className="mt-4 flex flex-wrap gap-3"><button onClick={() => decideProposal("approved")} className="inline-flex items-center gap-2 rounded-2xl bg-emerald-600 px-5 py-3 text-sm font-bold text-white"><CheckCircle2 size={16} /> Approve</button><button onClick={() => decideProposal("revision")} className="inline-flex items-center gap-2 rounded-2xl bg-rose-600 px-5 py-3 text-sm font-bold text-white"><XCircle size={16} /> Request revision</button></div></div></> : <p className="text-slate-500">No supervised projects yet.</p>}</div>
            ...
            <Card title="Announcements" icon={Megaphone}><div className="grid gap-3 md:grid-cols-3"><select value={announcementForm.targetGroup} onChange={(event) => setAnnouncementForm((prev) => ({ ...prev, targetGroup: event.target.value }))} className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm outline-none"><option value="all">Entire class</option>{groups.map((group) => <option key={group.name} value={group.name}>{group.name}</option>)}</select><input value={announcementForm.title} onChange={(event) => setAnnouncementForm((prev) => ({ ...prev, title: event.target.value }))} placeholder="Title" className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm outline-none" /><button onClick={sendAnnouncement} className="rounded-2xl bg-[#F34F1F] px-5 py-3 text-sm font-bold text-white">Broadcast</button></div><textarea value={announcementForm.message} onChange={(event) => setAnnouncementForm((prev) => ({ ...prev, message: event.target.value }))} rows={4} placeholder="Type announcement for all groups or one group..." className="mt-3 w-full rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm outline-none focus:border-[#F34F1F]" /><div className="mt-5 space-y-3">{announcements.map((item) => <div key={item.id} className="rounded-2xl bg-slate-50 p-4 text-sm"><p className="font-bold text-slate-900">{item.title}</p><p className="mt-1 text-slate-500">{item.message}</p><p className="mt-2 text-xs text-slate-400">To: {item.targetGroup} • {item.date}</p></div>)}</div></Card>
          </div>
        </div>
      </section>
    </main>
